...
eventSchema.methods.addAttendee = function(userId: string) {
  const existingAttendee = this.attendees.find(attendee => attendee.userId.toString() === userId);
  if (existingAttendee) {
...
eventSchema.methods.removeAttendee = function(userId: string) {
  const attendeeIndex = this.attendees.findIndex(attendee => attendee.userId.toString() === userId);
  if (attendeeIndex === -1) {
...
eventSchema.methods.addFeedback = function(userId: string, rating: number, comment?: string) {
  const existingFeedback = this.feedback.find(feedback => feedback.userId.toString() === userId);
  if (existingFeedback) {
...
eventSchema.statics.findByType = function(type: EventType) {
  return this.find({ type }).populate('organizer', 'firstName lastName email profilePicture');
};
