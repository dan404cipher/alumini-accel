...
const Events: React.FC = () => {
  ...
  const [selectedType] = useState<"reunion" | "webinar" | "all">("all");

  const upcomingEvents = events.filter(
    (e) => e.upcoming && (selectedType === "all" || e.type.toLowerCase() === selectedType)
  );
  ...
  return (
    <div className="p-8">
      ...
      {/* Upcoming Events */}
      <h2 className="text-3xl font-bold mb-8 pt-5" style={{ fontFamily: "TimesNewRoman" }}>
        Upcoming Events
      </h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16 cursor-pointer">
        {upcomingEvents.map((event) => (
          <div
            key={event.id}
            onClick={() => {
              if (event.type.toLowerCase() === "reunion") {
                navigate(`/reunion/${event.id}?type=reunion`);
              } else if (event.type.toLowerCase() === "webinar") {
                navigate(`/webinar/${event.id}?type=webinar`);
              }
            }}
      ...
      {/* Past Events */}
      <h2 className="text-3xl font-bold mb-8" style={{ fontFamily: "TimesNewRoman" }}>
        Past Events
      </h2>
