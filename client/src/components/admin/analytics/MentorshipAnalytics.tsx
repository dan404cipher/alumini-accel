...
import { Handshake, CheckCircle, Clock, Users } from "lucide-react";
...
export const MentorshipAnalytics = ({ data }: MentorshipAnalyticsProps) => {
  // Calculate summary statistics
  const totalMatches = data.byStatus.reduce((sum, item) => sum + item.count, 0);
  const activeMatches =
  ...
  const statusDataWithPercent = data.byStatus.map((item) => {
    const percent = totalMatches > 0 ? ((item.count / totalMatches) * 100).toFixed(0) : "0";
    return {
      ...item,
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="border-l-4 border-l-blue-500 bg-blue-50/50">
            ...
            <CardContent>
              <div className="text-2xl sm:text-3xl font-bold">{totalMatches}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Mentorship by Status - Pie Chart */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold">Mentorship Matches by Status</h3>
            <ChartContainer
              config={{
                count: {
                  label: "Count",
          ...
          {/* Mentorship Timeline - Line Chart */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold">
              Mentorship Matches Timeline (Last 12 Months)
            </h3>
            <ChartContainer
              config={{
                count: {
