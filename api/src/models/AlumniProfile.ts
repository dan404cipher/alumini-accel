...
alumniProfileSchema.methods.getCurrentPosition = function() {
  const currentJob = this.careerTimeline.find(job => job.isCurrent);
...
alumniProfileSchema.methods.addCareerEntry = function(entry: any) {
  ...
  this.careerTimeline.forEach(job => {
    if (entry.isCurrent) {
      job.isCurrent = false;
    }
  });
...
alumniProfileSchema.statics.findByBatchYear = function(batchYear: number) {
  return this.find({ batchYear }).populate('user', 'firstName lastName email profilePicture');
};

// Static method to find hiring alumni
alumniProfileSchema.statics.findHiring = function() {
  return this.find({ isHiring: true }).populate('user', 'firstName lastName email profilePicture');
};

// Static method to find mentors
alumniProfileSchema.statics.findMentors = function() {
  return this.find({ availableForMentorship: true }).populate('user', 'firstName lastName email profilePicture');
};
