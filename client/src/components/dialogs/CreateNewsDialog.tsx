  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.imageFile) {
        ...
        response = await newsAPI.createNewsWithImage(formDataToSend);
      } else {
      ...
      if (response.success) {
        ...
        let errorMessage = response.message || "Failed to create news";
        if (response.errors) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a new news article to share with the alumni community.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Optional: Add an image to make your news more engaging (PNG,
                JPG, GIF up to 5MB)
              </p>
            </div>
